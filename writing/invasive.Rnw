...
\title{Contrasting responses to climate variability generate strong seasonal priority effects in determining the outcomes of competition amoung native and invasive forest herbs} %emwmar10 -- maybe it's because it's Friday or because I have always found this paper hard to title, but I have no much better ideas just now. But we could simplify:
% Contrasting responses to climate generate seasonal priority effects that determi\ne competitive outcomes between native and invasive forest herbs
% or 
...
%emwmar10 -- good, but I would shorten this section -- just make it inline, not enumerated and (this will make it longer) explain briefly the rationale for most of the choices. See choppy edits below:
We selected focal\linelabel{crit1} species based on several criteria. First, we were interested in species that were herbaceous, biennial or perennial plants occurring in temperate forest understories of eastern North America. Because we were interested in plant competition, we selected species with substantial evidence that the species co-occur (and thus potentially compete) in nature. Finally, given our aim of understanding seasonal priority effects through phenological advantage, we selected species with sufficiently different responses to environmental variation. %emwmar10 -- I feel the phrasing as the end here is more true to how picked species than the question above about different dormancy types ... 
\end{enumerate}
%emwmar10 -- Do we really need the below? This would be the easiest way to make this shorter.... Maybe the whole paragraph can just fit in the letter? 
To evaluate species for criteria 1 and 2, we relied on our own personal observations, expert opinion, published species inventories, and occurrence records \citep[e.g.,][]{Petri:2022tp}. We based our evaluations of species for criteria three on published germination studies, and conducted small, pilot germination assays of likely candidates. We originally selected five species for our study, but due to restrictions on research activity during the COVID-19 and seed availability, we opted for a high-resolution case study with two focal\linelabel{crit2} species. %emwmar10 -- I would just put this in the letter, though I don't have a strong sense of whether that's the best choice. 

\subsection*{Focal species}
Dames Rocket (\textit{Hesperis matronalis}) is a herbaceous biennial/perennial species in the Brasicaeceae family, originally from Eurasia, and introduced to North America in the 19th century \citep{Francis:2009wz}. It  can rapidly invade  meadows, forest edges and woodlands, forming thick stands and excluding native vegetation \citep{Francis:2009wz}. It is currently listed as a noxious or invasive weed is several states and provinces in the United States and Canada \citep{Susko:2008ut}. Honewort (\textit{Cryptotania canadensis}) is a herbaceous perennial in the Apiaceae family, native to forests and woodlands of eastern and central North America \citep{Hawkins:2007vb}. Observations of each species\linelabel{occ1} indicate that they co-occur widely in these regions (Fig. \ref{fig:map}).  
%emwmar10 -- I would move below to supp and cite supp above. In the supp you should make a (slightly) longer case that 24% is pretty high given how these data work, but here it reads as rather small and is not worth explaining here IMHO. 
% We queried\linelabel{occ1a}  a recently published database of more than 80,000 plant community survey plots conducted federal and state agencies \citep{Petri:2022tp} and found that of the 160 research plots invaded by \emph{H. matronalis}, \emph{C. canadensis} was present at 24\%, suggesting interactions between these species in the wild are common, meeting our second criteria of species\linelabel{occ2} selection.

%emwmar10 -- what about germination strategies instead of germination niche?
While their habitat requirements may be similar, the two species display a substantially different germination niche, making them a suitable model for our study. \textit{C. canadensis} seeds are classified with deep physiological dormancy and require a substantial period of cold moist stratification to release dormancy and initiate germination \citep{Baskin:1988um}. While some reports suggest that cold stratification enhances germination in \textit{H. matronalis} at cool incubation temperatures, several studies have demonstrated that fresh and after-ripened (dry-stored) seeds of \textit{H. matronalis} are capable of rapid and complete germination at a wide range of temperatures \citep{Susko:2008ut}. The contrasting\linelabel{crit3} dormancy types are likely common across co-occuring species\linelabel{dorman1}  \citep{Baskin:1988aa,Le-Stradic:2015tt}, such as the two that we studied, and can be critical for coexistence in disturbed ecosystems \citep{Schippers:2001tj}. While it is expected that seeds with weaker dormancy germinate before dormant seeds, the degree to which these patterns of phenological assembly can be modified by climate variation is an important frontier for community interactions in the context of climate\linelabel{crit4} change \citep{Walck2011}. 
%emwmar10 -- is this really true? I am not sure we need it and worry it may further annoy the reviewer, I would consider cutting it. See edits above, but leave them if you feel strongly of course (though I still suggest we tighten this part up). 
% "The contrasting\linelabel{crit3} dormancy types of our focal species was an important character to meet our third criteria of species selection, listed above." 
...
\subsection*{Environmental drivers of seasonal priority effects} %emwmar10  -- this section is great
...
%emwmar10 -- question: should t50 be $t_{50}$ throughout?
...
%emwmar10 -- was this edited at all for revision? If so, I think we should tweak it to make a little more sense (maybe move the last sentence up as a topic sentence: 'Despite these diffs, consistent strat responses across both expts suggest our findings are robust'). But if not edited during this revision, let's not touch it. 
...
%emwmar10 -- do we want a better transition here? Could be something like, 'while more research is needed, our results are promising for in the context of native plant establishment' ...
...
\begin{figure}[h!] %emwmar10 -- be sure to highlight scale of changes we made in part 1 letter to editor -- a new figure!
    \centering
\includegraphics[width=.95\textwidth]{..//figure/cchhmmap4.jpeg}
\caption{Occurrence records of \textit{C. canadensis} and \textit{H. matronalis} in Eastern North America. Occurance data was obtained from the Global Biodiversity Information Facility \citep[GBIF,][]{gbif}.} 
   \label{fig:map}
\end{figure}
